...
from dgl import DGLBlock, DGLGraph
from dgl import EidExcluder
import numpy as np
import dgl
import ctypes
...
def build_csr_struct_array(g,temp_map):
    ...
    for i, etype in enumerate(g.canonical_etypes):
        ...
        # 将子图转为CSC格式
        csc_indptr, csc_indices, csc_data = dgl.to_csc(subgraph)
        
        # 获取图的相关属性
        num_nodes = g.num_nodes(subgraph.src_ntype)  # 源节点类型的节点数
        src_ntype = subgraph.src_ntype  # 源节点类型名称
        dst_ntype = subgraph.dst_ntype  # 目标节点类型名称
        src_vtype=temp_map[src_ntype]
        dst_vtype=temp_map[dst_ntype]
        # 将数据转换为指针类型
        inptr = csc_indptr.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        indices = csc_indices.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        ...
        # 将 csr 结构体添加到数组中
        csr_array.append(csr_struct)
        empty_csr_struct_array.append(empty_heterograph_struct)
...
def _sample_neighbors(
    ...
    for ntype in g.ntypes:
        if ntype in nodes:
            nodes_all_types.extend(nodes[ntype])
            current_length = len(nodes[ntype])
        else:
            current_length = 0
        temp_map[ntype] = len(nodes_all_types)
    ...
    node_mapping = np.full(sum(g.num_nodes(ntype) for ntype in g.ntypes),  # 计算所有节点数的总和-1
                           dtype=np.int32)
    sampled_nodes = np.full(sum(g.num_nodes(ntype) for ntype in g.ntypes),  # 计算所有节点数的总和-1
                           dtype=np.int32)
    ...
    if fused:
        ...
        sampled_nodes_offsets.data_as(ctypes.POINTER(ctypes.c_int)),
        ...
        subgidx = build_dgl_heterograph_from_arrays(csr_array, len(fanout))
