...
def ddp_setup(rank, world_size):
    ...
    os.environ['MASTER_PORT'] = "29593" #要指定端口，动态生成会导致init_proess_group卡死
    # 初始化进程组
...
def demo_basic(rank, world_size, usr_emb, rel_emb, subgraph_info, args):
    ...
    src_node, dst_node = rows, cols  # 不再进行对称扩展
    ...
    print(1111)
    test_sampler = my_sampler_th_1.NeighborSampler(g, batch_size=32,
      expand_factor = 100, neighbor_type='in', shuffle=False, num_hops=args.n_iter, 
    ...
    for epoch in range(args.n_epochs):
        train_nodes_count = dict()  # 记录同一个训练节点重复出现多少次
        step = 0
        running_loss = 0.0
