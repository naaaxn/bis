...
void prepare_for_sample(
    ...
    for (int node = 0; node < num_nodes; ++node) {
        ...
        neighbor_count[node]=count-temp_count;
        temp_count=count;
        src_node[node+1]=src_node[node]+min(count,num_samples);
    }
}
...
#pragma omp parallel for
    for (int i = 0; i < n; ++i) {
        ...
        sampled_indptr[new_index] = node_place[old_index+1]-node_place[old_index];  // 将原 indptr 的值拷贝到新位置
    }

    // 计算新的累积 indptr 范围
#pragma omp parallel for
    for (int i = 1; i <=n; ++i) {
        sampled_indptr[i] += sampled_indptr[i - 1];
    }
}
...
void MultiLayerNeighborSampler(
    ...
    std::vector<int> seed_nodes_mapped(node_types, 0);
    ...
    for (int etype = 0; etype < fanouts_size; ++etype){
       ...
       temp_edges+=nodes_offset[lhs_node_type+1]-nodes_offset[lhs_node_type];
       ...
       prepare_for_sample(nodes_all_type+nodes_offset[lhs_node_type],csr_array[etype].inptr,csr_array[etype].indices,fanouts[etype],temp_edges,
       temp_indptr.data(),neighbor_count.data(),number_of_nodes_specific_type);
       std::vector<int> sampled_indptr(temp_edges+1,0);
       ...
       if(number_of_nodes_specific_type == 0 || fanouts[etype] == 0){
        ...
        avx_th_neighbor_edges_sampling(csr_array[etype].indices,neighbor_count.data(),temp_indptr.data(),nodes_all_type+nodes_offset[lhs_node_type],
...
#pragma omp barrier
       ...
       sample_heterograph_array[etype].num_nodes=sampled_indptr.size()-1;
    }
