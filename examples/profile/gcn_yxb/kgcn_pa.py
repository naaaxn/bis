...
def demo_basic(rank, world_size, args):
    ...
    # if(rank == 0):
    #     print(rank, "映射前的 df_kg:")
    #     print(df_kg)
    #     print(rank, "映射前的dataset：")
    #     print(df_dataset)

    # # 重新映射子图节省空间
    # unique_nodes = pd.concat([df_kg['head'], df_kg['tail']]).drop_duplicates().reset_index(drop=True)
    # continuous_ids = np.arange(len(unique_nodes))
    # node_to_id = dict(zip(unique_nodes, continuous_ids))

    # df_kg['head'] = df_kg['head'].map(node_to_id)
    # df_kg['tail'] = df_kg['tail'].map(node_to_id)
    # df_dataset['itemID'] = df_dataset['itemID'].map(node_to_id)

    # if(rank == 0):
    #     print(rank, "映射后的 df_kg:")
    #     print(df_kg)
    #     print(rank, "映射后的dataset：")
    #     print(df_dataset)
    ...
    for epoch in range(args.n_epochs):
        ...
        print('[Rank {}-Epoch {}]train_loss: '.format(rank, epoch+1), running_loss / len(train_loader))
        ...
        with torch.no_grad():
            ...
            print('[Rank {}-Epoch {}]test_loss: '.format(rank, epoch+1), test_loss / len(test_loader))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sub_graph_dataset', type=str, default='PaGraph/data/yxb_data_pagraph', help='which sub_graph_dataset to use')
