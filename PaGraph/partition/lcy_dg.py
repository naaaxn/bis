...
import pandas as pd
from KGCN_lcy.data_loader import DataLoader
import ordering
...
if __name__ == '__main__':
  ...
  # print("p_v:")
  # print(p_v) # 包含每个分区的节点集合？每个元素是一个numpy数组，表示一个子图的节点id
  # print("p_trainv:")
  # print(p_trainv) # 包含每个分区的训练节点集合?
  ...
  for pid, (pv, ptrainv, puser) in enumerate(zip(p_v, p_trainv, p_user)):
    ...
    subadj, sub2fullid, subtrainid, sub_kg = get_sub_graph(dgl_g, ptrainv, args.num_hop, kg_df)
    # print("subadj:")
    # print(subadj)
    # print("sub2fullid:")
    # print(sub2fullid)
    ...
    sub_label_file = os.path.join(
      ...
      # 保存kg子图为kgcn可以处理的dataframe格式
    sub_kg_file = os.path.join(
      partition_dataset,
      'sub_kg_{}.csv'.format(str(pid)))
    sub_kg.to_csv(sub_kg_file, sep='\t', index = False)   
