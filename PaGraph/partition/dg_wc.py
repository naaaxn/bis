...
def dg(partition_num, adj, train_nids, hops, item_df): # 执行图分区的主要函数，将图数据划分多个子图
  # print("coo_adj: ", adj.data.size)
  csc_adj = adj.tocsc()
  # print("csc_adj: ", csc_adj.data.size)
  vnum = adj.shape[0]
  vtrain_num = train_nids.shape[0]
  print("知识图谱节点数量: ", vnum)
  print("训练节点数量: ", vtrain_num)
...
if __name__ == '__main__':
  ...
  for pid, (pv, ptrainv, puser) in enumerate(zip(p_v, p_trainv, p_user)):
    ...
    kg_df_filtered = kg_df[(kg_df['head'].isin(pv)) | (kg_df['tail'].isin(pv))]
    print("分区后的kg：")
