...
class GraphCacheServer:
  ...
  def auto_cache(self, dgl_g, embed_names):
    ...
    # Step3: cache 如果可用内存足够大，就缓存整个图的特征数据，即 is_full=True。
    #如果可用内存不足，就根据一定的策略选择部分节点进行缓存。
    #这里的策略是选择出度（out_degrees）较高的节点进行缓存。根据出度排序后，选择前 capability 个节点进行缓存，即 is_full=False。
