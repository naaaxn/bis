...
import pandas as pd
# 读入训练集
def get_sub_train_dataset(dataname, idx, partitions):
  dataname = os.path.join(dataname, '{}naive'.format(partitions))
  sub_train_file = os.path.join(dataname, 'sub_user_{}.csv'.format(idx))
  df_dataset = pd.read_csv(dataname, sep='\t')
  return df_dataset
...
def get_sub_train_graph(dataname, idx, partitions, dataset):
  """
  Params:
  ...
  Returns:
    ...
    train2fullid
  """
  dataname = os.path.join(dataname, '{}naive_{}'.format(partitions, dataset))
...
def get_sub_train_nid(dataname, idx, partitions, dataset):
  dataname = os.path.join(dataname, '{}naive_{}'.format(partitions, dataset))
