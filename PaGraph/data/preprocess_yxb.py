...
#   pp = np.loadtxt(filepath, delimiter=delimiter)
  pp = np.genfromtxt(filepath, delimiter='\t', usecols=(0, 2), dtype=int)
#   print("pp.txt: ", pp)
  print("kg.txt: ", pp)
  src_node = pp[:,0].astype(np.int32)
  dst_node = pp[:,1].astype(np.int32)
  ...
  print("edge_weight: ")
  print(edge_weight)
  print("edge_weight_length: ", edge_weight.shape[0])
  print("src_node_length: ", src_node.shape[0])
  print("dst_node_length: ", dst_node.shape[0])
...
def split_dataset(vnum, outdir=None):
  ...
  # train mask
  train_mask = np.zeros(vnum, dtype=np.int32)
  train_mask[nids[0:train_len]] = 1
  # val mask
  val_mask = np.zeros(vnum, dtype=np.int32)
  val_mask[nids[train_len:train_len + val_len]] = 1
  # test mask
  test_mask = np.zeros(vnum, dtype=np.int32)
...
if __name__ == '__main__':
  ...
  print('Done.')
  temp = np.load(os.path.join(args.dataset, 'labels.npy'))
#   print(temp)